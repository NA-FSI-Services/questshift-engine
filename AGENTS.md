...
- Keep committed `questshift.llm.api-key=none`. Put a real key only in untracked `application-local.properties` (copy `application-local.properties.example`) or the cluster, never in git. Committed model stays `ibm-granite/granite-3.2-8b-instruct`.
